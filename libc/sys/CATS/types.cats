...
atslib_eq_dev_dev
  (dev_t x1, dev_t x2) {
  return ((memcmp(&x1, &x2, sizeof(dev_t))==0) ? ats_true_bool : ats_false_bool) ;
...
typedef
fsblkcnt_t ats_fsblkcnt_type ; // file system block counts

typedef
fsfilcnt_t ats_fsfilcnt_type ; // file system file counts
...
atslib_eq_ino_ino
  (ino_t x1, ino_t x2) {
  return ((memcmp(&x1, &x2, sizeof(ino_t))==0) ? ats_true_bool : ats_false_bool) ;
