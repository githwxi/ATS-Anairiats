...
//
void timeout_add_val (uint ntime) {
  if (timeout_id == 0) {
...
//
void timeout_add () {
  timeout_add_val (TIMEOUT_INTERVAL) ; return ;
} // end of [timeout_add]
//
void timeout_remove () {
  if (timeout_id != 0) {
...
extern fun timeout_add_val (ntime: uint): void = "timeout_add_val"
...
//
    ...
    val () = if ~(isRot) then let
      ...
      val () = timeout_remove ()
      val () = timeout_add_val (uint_of(10 * TIMEOUT_INTERVAL))
    in
      !theAlpha_ref := 0.0
    end // end of [val]
  } // end of [if]
...
//
val () = theSlideCount_set (1) // starting from the first slide
//
