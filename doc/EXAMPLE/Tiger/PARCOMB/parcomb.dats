...
implement{t} stream_item_get (tks) = case+ !tks of
...
implement{a}{t} apply_parser (p, tks, ncur, nmax) = p (tks, ncur, nmax)

(* ****** ****** *)

implement lzeta (p) = lam (tks, ncur, nmax) => !p (tks, ncur, nmax)

(* ****** ****** *)

implement{a}{t} return_parser (x) = lam (tks, ncur, nmax) => Okay (x)

(* ****** ****** *)

implement{a}{t}
  alt_parser_parser (p1, p2) = lam (tks, ncur, nmax) => let
  val tks0 = tks
  val ncur0 = ncur
...
implement{a1,a2}{t}
  seq_parser_parser (p1, p2) = lam (tks, ncur, nmax) => let
...
implement{a1,a2,b}{t}
  seq2wth_parser_fun (p1, p2, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let
      val r2 = p2 (tks, ncur, nmax) in case+ r2 of
...
implement{a1,a2,b}{t}
  seq2wth_parser_cloref (p1, p2, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let
      val r2 = p2 (tks, ncur, nmax) in case+ r2 of
    | ~Okay v2 => Okay (f (v1, v2)) | Fail _ => (fold@ r2; r2)
...
implement{a1,a2,a3,b}{t}
  seq3wth_parser_fun (p1, p2, p3, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let
      val r2 = p2 (tks, ncur, nmax) in case+ r2 of
      | ~Okay v2 => let
...
implement{a1,a2,a3,b}{t}
  seq3wth_parser_cloref (p1, p2, p3, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let
      val r2 = p2 (tks, ncur, nmax) in case+ r2 of
      | ~Okay v2 => let
...
implement{a1,a2,a3,a4,b}{t}
  seq4wth_parser_fun (p1, p2, p3, p4, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
...
implement{a1,a2,a3,a4,b}{t}
  seq4wth_parser_cloref (p1, p2, p3, p4, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let val r2 = p2 (tks, ncur, nmax) in case+ r2 of
    | ~Okay v2 => let val r3 = p3 (tks, ncur, nmax) in case+ r3 of
      | ~Okay v3 => let val r4 = p4 (tks, ncur, nmax) in case+ r4 of
...
implement{a1,a2,a3,a4,a5,b}{t}
  seq5wth_parser_fun (p1, p2, p3, p4, p5, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let val r2 = p2 (tks, ncur, nmax) in case+ r2 of
    | ~Okay v2 => let val r3 = p3 (tks, ncur, nmax) in case+ r3 of
      | ~Okay v3 => let val r4 = p4 (tks, ncur, nmax) in case+ r4 of
...
implement{a1,a2,a3,a4,a5,b}{t}
  seq5wth_parser_cloref (p1, p2, p3, p4, p5, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let val r2 = p2 (tks, ncur, nmax) in case+ r2 of
    | ~Okay v2 => let val r3 = p3 (tks, ncur, nmax) in case+ r3 of
      | ~Okay v3 => let val r4 = p4 (tks, ncur, nmax) in case+ r4 of
...
implement{a1,a2,a3,a4,a5,a6,b}{t}
  seq6wth_parser_fun (p1, p2, p3, p4, p5, p6, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let val r2 = p2 (tks, ncur, nmax) in case+ r2 of
    | ~Okay v2 => let val r3 = p3 (tks, ncur, nmax) in case+ r3 of
      | ~Okay v3 => let val r4 = p4 (tks, ncur, nmax) in case+ r4 of
...
implement{a1,a2,a3,a4,a5,a6,b}{t}
  seq6wth_parser_cloref (p1, p2, p3, p4, p5, p6, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let val r2 = p2 (tks, ncur, nmax) in case+ r2 of
    | ~Okay v2 => let val r3 = p3 (tks, ncur, nmax) in case+ r3 of
      | ~Okay v3 => let val r4 = p4 (tks, ncur, nmax) in case+ r4 of
...
implement{a1,a2,a3,a4,a5,a6,a7,b}{t}
  seq7wth_parser_fun (p1, p2, p3, p4, p5, p6, p7, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let val r2 = p2 (tks, ncur, nmax) in case+ r2 of
    | ~Okay v2 => let val r3 = p3 (tks, ncur, nmax) in case+ r3 of
      | ~Okay v3 => let val r4 = p4 (tks, ncur, nmax) in case+ r4 of
...
implement{a1,a2,a3,a4,a5,a6,a7,b}{t}
  seq7wth_parser_cloref (p1, p2, p3, p4, p5, p6, p7, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let val r2 = p2 (tks, ncur, nmax) in case+ r2 of
    | ~Okay v2 => let val r3 = p3 (tks, ncur, nmax) in case+ r3 of
      | ~Okay v3 => let val r4 = p4 (tks, ncur, nmax) in case+ r4 of
...
implement{a1,a2,a3,a4,a5,a6,a7,a8,b}{t}
  seq8wth_parser_fun (p1, p2, p3, p4, p5, p6, p7, p8, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let val r2 = p2 (tks, ncur, nmax) in case+ r2 of
    | ~Okay v2 => let val r3 = p3 (tks, ncur, nmax) in case+ r3 of
      | ~Okay v3 => let val r4 = p4 (tks, ncur, nmax) in case+ r4 of
...
implement{a1,a2,a3,a4,a5,a6,a7,a8,b}{t}
  seq8wth_parser_cloref (p1, p2, p3, p4, p5, p6, p7, p8, f) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
  | ~Okay v1 => let val r2 = p2 (tks, ncur, nmax) in case+ r2 of
    | ~Okay v2 => let val r3 = p3 (tks, ncur, nmax) in case+ r3 of
      | ~Okay v3 => let val r4 = p4 (tks, ncur, nmax) in case+ r4 of
...
implement{a1,a2}{t}
  proj1_parser_parser (p1, p2) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax)
in
  case+ r1 of
  | Okay _(*v1*) => let 
...
implement{a1,a2}{t}
  proj2_parser_parser (p1, p2) = lam (tks, ncur, nmax) => let
  val r1 = p1 (tks, ncur, nmax) in case+ r1 of
...
implement{a}{t}
  sat_parser_fun (p, f) = lam (tks, ncur, nmax) => let
...
implement{a}{t}
  sat_parser_cloref (p, f) = lam (tks, ncur, nmax) => let
  val tks0 = tks and ncur0 = ncur
  val r = p (tks, ncur, nmax) in case+ r of
  | Okay v => begin
      if f v then (fold@ r; r) else begin
...
implement{a} any_parser () = lam (tks, ncur, nmax) => let
...
implement{a} anyopt_parser () = lam (tks, ncur, nmax) => let
  val ans = stream_item_get<a> tks in case+ ans of
...
implement{} fail_parser () = lam (tks, ncur, nmax) => Fail ()

(* ****** ****** *)

// it is difficult to know where the failure occurs!
implement{a}{t} neg_parser (p) = lam (tks, ncur, nmax) => let
  val tks0 = tks and ncur0 = ncur
...
implement{a}{t} repeat0_parser (p) = lam (tks, ncur, nmax) => let
  typedef T = List a
  fun loop (
...
implement{a}{t}
  repeat1_parser (p) = lam (tks, ncur, nmax) => let
  val r = p (tks, ncur, nmax) in case+ r of
  | ~Okay v => let
...
implement{a,b}{t}
  repeat0_sep_parser (p, sep) = lam (tks, ncur, nmax) => let
  val tks0 = tks and ncur0 = ncur
...
implement{a,b}{t}
  repeat1_sep_parser (p, sep) = lam (tks, ncur, nmax) => let
  val r = p (tks, ncur, nmax) in case+ r of
  | ~Okay v0 => let
...
implement{a}{t}
  discard_one_parser (p) = lam (tks, ncur, nmax) => let
  val r = p (tks, ncur, nmax) in case+ r of
  | ~Okay v => Okay unit | Fail _ => (fold@ r; r)
end // end of [discard_parser]

implement{a}{t}
  discard_many_parser (p) = lam (tks, ncur, nmax) => let
  fun loop (
