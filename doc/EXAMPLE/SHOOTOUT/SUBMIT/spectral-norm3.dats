...
**
** compilation command:
**   atscc -D_GNU_SOURCE -D_ATS_MULTITHREAD -O3 -msse2 spectral-norm3.dats -o spectral-norm3 -lm -lpthread
**
*)
...
//
  val nworker =
    workshop_get_nworker (ws)
  val () = assertloc (nworker > 0)
  val INC = (N+nworker-1)/nworker/64
  val INC = max (1, (int1_of_int)INC)
  val () = assert_errmsg_bool1 (INC > 0, #LOCATION)
