...
This implementation handily beats a corresponding one in C
(because GC is good for handling small memory allocation and
deallocation)
...
fun make (d: int, i: int): Tree = let
(*
  val () = printf ("make (%i, %i)", @(d, i))
*)
in
  if d > 0 then
  ...
  if d <= max_depth then let
    ...
    fun loop (i: Nat, c: int):<cloptr1> int =
      if i < n then
        let
          val t = make(d, i)
          val c1 = check_and_free t
          val t = make(d, ~i)
          val c2 = check_and_free t
        in
          loop (i+1, c + c1 + c2)
        end
      else c
...
// val () = (prerr "[stretch] is done"; prerr_newline ())
val long_lived_tree = long_lived_tree_make (max_depth)
// val () = (prerr "[long_live_tree] is done"; prerr_newline ())
