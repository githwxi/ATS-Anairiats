...
scullc_read_main (
  int m
, int n
, struct scullc_qset *xs
, int ln
, int i, int j
, char __user *pbf
, int cnt
, loff_t *f_pos
) ; // end of [scullc_read_main]
ssize_t
scullc_read (
  struct file *filp
...
int scullc_trim(struct scullc_dev *dev)
{
	...
	int qset = dev->m_qset;   /* "dev" is not-null */
	int i;
	...
	dev->size = 0;
	dev->m_qset = scullc_qset;
	dev->n_quantum = scullc_quantum;
	dev->next = NULL;
	return 0;
}
#endif // end of [if(0)]
...
void scullc_trim_main (
  struct scullc_dev *dev, int m, int n
) ; // end of [scullc_trim_main]
int scullc_trim(struct scullc_dev *dev) {
  scullc_trim_main (dev, scullc_qset, scullc_quantum) ; return 0 ;
} // end of [scullc_trim]
...
int scullc_init(void)
{
	...
	for (i = 0; i < scullc_devs; i++) {
		scullc_devices[i].n_quantum = scullc_quantum;
		scullc_devices[i].m_qset = scullc_qset;
