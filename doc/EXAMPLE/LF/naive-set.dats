...
extern
praxi inter_elim1 {X,Y:set}
  {x:elt} (pf: IN (x, X*Y)): IN (x, X)

extern
praxi inter_elim2 {X,Y:set}
  {x:elt} (pf: IN (x, X*Y)): IN (x, Y)

(* ****** ****** *)

prfn union_assoc
  {X,Y,Z:set} (): SETEQ ((X+Y)+Z, X+(Y+Z)) = let
  stadef XY_Z = (X+Y)+Z and X_YZ = X+(Y+Z)
  prfn f {x:elt} (pf: IN (x, XY_Z)): IN (x, X_YZ) = begin
    case+ union_elim {X+Y,Z} (pf) of
    | inl pfXY => begin case+ union_elim {X,Y} (pfXY) of
      | inl pfX => union_intr1 {X,Y+Z} (pfX)
      | inr pfY => union_intr2 {X,Y+Z} (union_intr1 {Y,Z} (pfY))
      end // end of [inl]
    | inr pfZ => union_intr2 {X,Y+Z} (union_intr2 {Y,Z} (pfZ))
  end // end of [f]
  prfn g {x:elt} (pf: IN (x, X_YZ)): IN (x, XY_Z) = begin
    case+ union_elim {X,Y+Z} (pf) of
    | inl pfX => union_intr1 {X+Y,Z} (union_intr1 {X,Y} (pfX))
    | inr pfYZ => begin case+ union_elim {Y,Z} (pfYZ) of
...
prfn inter_assoc
  {X,Y,Z:set} (): SETEQ ((X*Y)*Z, X*(Y*Z)) = let
  stadef XY_Z = (X*Y)*Z and X_YZ = X*(Y*Z)
  prfn f {x:elt} (pf: IN (x, XY_Z)): IN (x, X_YZ) = let
  ...
  prfn g {x:elt} (pf: IN (x, X_YZ)): IN (x, XY_Z) = let
...
prfn inter_union_distribute
  {X,Y,Z:set} (): SETEQ (X*(Y+Z), X*Y+X*Z) = let
  prfn f {x:elt} (pf: IN (x, X*(Y+Z))): IN (x, X*Y+X*Z) = let
    prval pfX = inter_elim1 {X,Y+Z} (pf)
  ...
  prfn g {x:elt} (pf: IN (x, X*Y+X*Z)): IN (x, X*(Y+Z)) = begin
