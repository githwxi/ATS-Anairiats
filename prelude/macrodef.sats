...
macdef
orelse (x, y) = if ,(x) then true else ,(y)
macdef
...
fun
stradd :
  (string, string) -> string = "atspre_string_append"
//
in (* in-of-local *)

macdef
exitloc (x) = exit_errmsg (,(x), stradd(#LOCATION,"\n"))
macdef
assertloc (x) = assert_errmsg (,(x), stradd(#LOCATION,"\n"))
...
macrodef
rec
power_mac x(*base:code*) n(*exponent:int*) =
