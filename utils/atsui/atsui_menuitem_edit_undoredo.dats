...
typedef struct {
  gchar cmd ;
  gint _beg ; gint _end ;
  gboolean seq ; // sequence flag
  gchar *str ; // a linear gstring
} DOINFO_t ;
typedef GList DOINFOLST_t ;

ATSinline()
ats_void_type
atsui_doinfoptr_free
  (ats_ptr_type x) {
  if (x) {
    g_free (((DOINFO_t*)x)->str) ; g_free (x) ;
  } // end of [if]
  return ;
} // end of [atsui_doinfoptr_free]
%} // end of [%{^]
...
viewtypedef DOINFO =
  $extype_struct "DOINFO_t" of {
  cmd= gchar
, _beg= gint, _end= gint
, seq= gboolean // sequence flag
, str= gstring0 // a linear gstring
} // end of [DOINFO]
absviewtype DOINFOptr (l:addr)
extern fun doinfoptr_free {l:addr} (x: DOINFOptr l): void
  = "atsui_doinfoptr_free"
// end of [atsui_doinfoptr_free]

(* ****** ****** *)

absviewtype DOINFOLSTptr (l:addr)
abst@ype
DOINFOLSTrest // totally opaque but nonlinear
viewtypedef
DOINFOLST (l1:addr, l2:addr) =
  $extype_struct "DOINFOLST_t" of {
  data= DOINFOptr l1, next= DOINFOLSTptr l2
, _rest= DOINFOLSTrest
} // end of [DOINFOLST]
...
extern
castfn ptr_of_doinfolstptr {l:addr} (xs: !DOINFOLSTptr l): ptr l
overload ptr_of with ptr_of_doinfolstptr

extern
castfn doinfolst_fold {l,l1,l2:addr | l1 > null}
  (pf1: gfree_v l, pf2: DOINFOLST (l1, l2) @ l | p_xs: ptr l): DOINFOLSTptr l
// end of [doinfolst_fold]

extern
castfn doinfolst_unfold {l:agz} (xs: DOINFOLSTptr l)
  : [l1,l2:addr | l1 > null] (gfree_v l, DOINFOLST (l1, l2) @ l | ptr l)
// end of [doinfolst_unfold]

(* ****** ****** *)

macdef INS = 0 // insert
macdef BSP = 1 // backspace
macdef DEL = 2 // delete

(* ****** ****** *)

fun doinfolstptr_free
  {l:addr} (xs: DOINFOLSTptr l): DOINFOLSTptr null = let
  val p_xs = ptr_of (xs)
  prval () = ptr_is_gtez (p_xs)
in
  if p_xs > null then let
    val [l1,l2:addr] (pf1, pf2 | p_xs) = doinfolst_unfold (xs)
    viewtypedef T = DOINFOLST (null, null)
    val () = doinfoptr_free (p_xs->data)
    val xs = p_xs->next
    val () = gptr_free {T} (pf1, pf2 | p_xs)
  in
    doinfolstptr_free (xs)
  end else xs
end // end of [doinflstptr_free]

(* ****** ****** *)

%{^
static gboolean the_seq_reserve = FALSE ;
gboolean
atsui_the_seq_reserve_get
  () { return the_seq_reserve ; }
void
atsui_the_seq_reserve_set
  (gboolean seq) { the_seq_reserve = seq ; return ; }
%} // end of [%{^]
extern
fun the_seq_reserve_get
  (): gboolean = "atsui_the_seq_reserve_get"
// end of [the_seq_reserve_get]
extern
fun the_seq_reserve_set
  (seq: gboolean): void = "atsui_the_seq_reserve_set"
// end of [the_seq_reserve_set]

(* ****** ****** *)

extern
fun doinfoptr_make (
  cmd: gchar, _beg: gint, _end: gint, seq: gboolean, str: gstring0
) : [l:agz] DOINFOptr (l)
...
static void undo_append_undo_info
  (GtkTextBuffer *buffer, gchar command, gint start, gint end, gchar *str)
{
...
extern fun theUndoWidget_get ()
  : [l:agz] (gobjref (GtkWidget, l) -<lin,prf> void | gobjref (GtkWidget, l))
// end of [theUndoWidget_get]

extern fun theRedoWidget_get ()
  : [l:agz] (gobjref (GtkWidget, l) -<lin,prf> void | gobjref (GtkWidget, l))
// end of [theRedoWidget_get]

(* ****** ****** *)

(* end of [atsui_menuitem_edit_undoredo.dats] *)
