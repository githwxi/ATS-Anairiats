...
*)
  val () = if (!toggle_ref = 1) then timeout_add () in GTRUE
...
*)
  val () = if (!toggle_ref = 1) then timeout_remove () in GTRUE
...
//
val () = if !toggle_ref = 1 then timeout_add () // start the clock
//
} // end of [main1]
