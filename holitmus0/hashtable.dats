...
extern fun{key:t@ype;itm:viewt@ype} hashtbl_foreach_cloptr {v:view}
  (pf: !v | tbl: hashtbl_t (key, itm), f: !(!v | key, &itm) -<cloptr1> void): void
...
fun{key:t@ype;itm:viewt@ype}
  chain_foreach_cloptr {v:view} {n:nat} {f:eff} .<n>. (
    pf: !v | kis: !chain (key, itm, n), f: !(!v | key, &itm) -<cloptr,f> void
  ) :<f> void = begin case+ kis of
  | cons (k, !i, !kis1) => begin
      f (pf | k, !i); chain_foreach_cloptr (pf | !kis1, f); fold@ kis
    end // end of [cons]
  | nil () => fold@ kis
end // end of [chain_foreach_cloptr]
...
fn{key:t@ype;itm:t@ype} hashtbl_ptr_search_off
  ...
  | p_beg: ptr l_beg, k0: key, eq: eq key, off: int off
  ) :<> Option_vt itm = let
  val (pf1, pf2 | p_mid) =
...
fn{key:t@ype;itm:viewt@ype} hashtbl_ptr_insert_off
  ...
  | p_beg: ptr l_beg, k: key, i: itm, off: int off
  ) :<> void = let
  val (pf1, pf2 | p_mid) =
    hashtbl_split<> {..} {sz,off,tot} (pf | p_beg, off)
  prval hashtbl_v_cons (pf21, pf22) = pf2
...
fun{key:t@ype;itm:viewt@ype}
  ...
  | ~cons (k, i, kis) => let
      ...
      val h = uint1_of_uint (h); val [off:int] off = h mod sz
      val (pf1, pf2 | p_mid) =
...
in
  ...
  | hashtbl_vt_some (_, !pf | sz, _, p_beg, hash, eq) => let
      ...
      val h = uint1_of_uint (h); val off = h mod sz
...
implement{key,itm}
  hashtbl_foreach_cloptr {v} (pf0 | tbl, f) = let
  val (vbox pf_tbl | p_tbl) = ref_get_view_ptr (tbl)
in
  case+ !p_tbl of
  | hashtbl_vt_some (_, !pf | sz, _, p_beg, _, _) => let
      val () = $effmask_ref begin
        hashtbl_ptr_foreach_cloptr {v} (pf0, !pf | sz, p_beg, f)
      end
    in
      fold@ !p_tbl
    end // end of [cons]
  | hashtbl_vt_none () => fold@ !p_tbl
end // end of [hashtbl_foreach_cloptr]

(* ****** ****** *)

(*
** HX (2008-Oct-15):
**   is there a better approach that is also practical?
*)
implement{key,itm}
  ...
  val () = hashtbl_foreach_cloptr<key,itm> {v} (pf0 | tbl, f)
